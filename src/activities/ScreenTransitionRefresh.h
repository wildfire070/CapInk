...
// X4-class panels need a clean waveform for those transitions; repeated renders
// within the same state (progress, signal strength, selection) remain fast.
class ScreenTransitionRefresh {
 public:
  HalDisplay::RefreshMode modeFor(const uint8_t screenState) {
    ...
    hasRendered_ = true;
    return screenChanged && !gpio.deviceIsX3() ? HalDisplay::FULL_REFRESH : HalDisplay::FAST_REFRESH;
  }
